...
app.secret_key = os.environ.get('SECRET_KEY', 'Redluckyxyz-hosting-secret-2026')
ADMIN_USERNAME = 'Redluckyxyz'
ADMIN_PASSWORD = 'Redluckyxyz1'

# Deployment storage:
# Vercel functions have an ephemeral, writable /tmp filesystem.
# For persistent users/bot files, set DATA_ROOT to an external mounted/persistent path
# or replace the JSON storage with a database/object store.
IS_VERCEL = bool(os.environ.get('VERCEL'))
DATA_ROOT = os.environ.get(
    'DATA_ROOT',
    '/tmp/lucky_hosting_data' if IS_VERCEL else os.path.join(os.getcwd(), 'data')
)
app.config['MAX_CONTENT_LENGTH'] = 50 * 1024 * 1024

USERS_FILE = os.path.join(DATA_ROOT, 'users.json')
BOTS_DIR = os.path.join(DATA_ROOT, 'bots')
CPU_HISTORY = {}
CRASH_COUNT = {}
NET_STATS = {}
...
# Vercel is a serverless platform. Long-running child processes, background threads,
# and local filesystem persistence are not supported as a permanent bot-hosting runtime.
VERCEL_RUNTIME_MESSAGE = (
    'Bot start/stop and persistent file storage require a long-running server or '
    'external worker/storage. The web panel itself is Vercel-compatible.'
)
...
def create_default_files(server_dir):
    ...
    if not os.path.exists(main_py):
        with open(main_py, 'w', encoding='utf-8') as f:
            f.write('''# LUCKY HOSTING - Default Bot
import time

print("=" * 40)
print("Bot is running on LUCKY HOSTING")
...
def api_create_server():
    ...
    if not username:
        username = f"RED_LUCKY_XYZ{random.randint(10000, 99999)}"
...
def api_run(server_id):
    if IS_VERCEL:
        return jsonify({'status': 'error', 'msg': VERCEL_RUNTIME_MESSAGE, 'vercel_compatible': True}), 501
    server, _ = get_server_by_id(server_id)
...
def api_command():
    if IS_VERCEL:
        return jsonify({'status': 'error', 'msg': 'Web console commands are disabled on Vercel serverless runtime.'}), 501
    data = request.get_json()
    cmd = data.get('cmd', '')
